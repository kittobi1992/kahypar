...
static inline double imbalance(const Hypergraph& hypergraph, const Context& context) {
  ...
  HypernodeWeight perfect_balance_part_weights = 0;
  if (!context.partition.use_individual_part_weights) {
    perfect_balance_part_weights = ceil(hypergraph.totalWeight()
                                        / static_cast<double>(context.partition.k));
  } else {
    perfect_balance_part_weights  = context.partition.perfect_balance_part_weights[0];
  }
  ...
  for (PartitionID i = 1; i != context.partition.k; ++i) {
    ...
    const double balance_i =
        (hypergraph.partWeight(i) /  static_cast<double>((context.partition.use_individual_part_weights ?
